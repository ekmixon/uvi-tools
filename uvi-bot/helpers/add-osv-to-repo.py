...
for i in all_ids:
    ...
    if "OSV" not in the_data:
        ...
        if (
            "extended_references" not in issue_data
            and issue_data["vendor_name"] == "Linux"
            and issue_data["product_name"] == "Kernel"
        ):
        ...
        new_time = f"{datetime.datetime.fromtimestamp(ctime).isoformat()}Z"
